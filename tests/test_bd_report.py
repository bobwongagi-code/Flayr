...
class BdReportTests(unittest.TestCase):
    ...
    def test_projection_matches_internal_report_contract(self) -> None:
        ...
        self.assertEqual(data["gates"][0]["evidenceStrength"], "待确认")
        ...
        self.assertEqual(data["improvements"][0]["priorityLabel"], "高优先级")
        self.assertEqual(data["improvements"][0]["impactDirection"], "潜在影响较大")
        self.assertEqual(data["improvements"][0]["evidenceStrength"], "中")
        ...
        self.assertEqual(data["metadata"]["template_version"], "bd-internal-v2")
        self.assertTrue(data["metadata"]["generated_by"])
        serialized = json.dumps(data, ensure_ascii=False)
        self.assertNotIn("gmvImpact", serialized)
        self.assertNotIn("gmvClass", serialized)
    ...
    def test_template_is_runtime_bound_and_escapes_script_data(self) -> None:
        ...
        self.assertIn("TOP 3 提升点 · 按商业优先级排序", template)
        self.assertNotIn("GMV 影响", template)
        self.assertNotIn("P0", template)
