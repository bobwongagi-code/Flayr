...
| `subtitle_track.json`（OCR 权威字幕轨） | Agent Plan 豆包视觉模型，auto 策略（有分析 key 即开） | 可选 | 状态标记，缺失显示占位 |
...
| `llm/payload.py` | `build_*_payload` 系列。阶段一 `build_video_fact_payload`（原生视频直传）；阶段二 `build_llm_comparison_payload` + `build_evidence_sensory_inputs`（Agent Plan 每条 evidence 配带原声短视频，旧兼容服务保留关键帧+切片音频）；Phase C `build_stage_review_payload`（低置信阶段原生视频切片）。 |
...
ffmpeg 不可用时阶段一降级为关键帧。支持独立音频输入且通过能力验证的兼容服务可直接观察音轨；Agent Plan 当前按“连续画面 + Whisper 转录 + 本地音频质检”运行，不得脑补语气、BGM或音效。
...
- 输出不合 schema 时按 provider repair：Doubao 最多两轮局部 JSON Pointer 补丁，Qwen/兼容模型保留一次完整 repair；局部补丁不得修改事实、证据引用、命题引用或 severity，仍失败则 fail loud。
