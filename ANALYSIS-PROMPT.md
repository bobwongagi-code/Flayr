...
4a. **两侧独立执行分**：对应 JSON 字段 `creator_execution` / `benchmark_execution`，**取值只能是 0、0.5、1、2 四个数字**：
   - **0 = 未执行**该阶段功能
   - **0.5 = 基本无效**：做了但对该阶段核心功能基本不起作用——敷衍、平庸无感（如一句轻带、不注意听不到的 CTA；平铺直叙毫无抓力的开场；仅口头承诺、没有任何验证支撑）。**"内容存在但消费者无法有效接收"同样按 0.5**：看不清（虚焦/过曝/遮挡/一闪而过）、听不清（吞字/被 BGM 压制）、读不完（字幕停留过短）——物理存在 ≠ 有效传递。**S5 背书孤证规则**：仅口播提及背书而画面无佐证、或背书标志一闪而过无法辨认，执行分最高 0.5
   - **1 = 合格**：功能完成且对观众有效
