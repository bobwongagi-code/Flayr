...
### 2.5.4 当前 S3 framing 规则状态

`process_framing_met` 用来记录 S3 使用过程是否拍全、拍清、对准，当前只作为 trace/审计字段，不直接影响 S3 severity。它可以帮助复盘“有使用动作但证据接收困难”的问题；是否升级为打分规则，必须等 holdout 样本证明它跨品类稳定有效后再决定。
