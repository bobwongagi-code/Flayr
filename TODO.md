...
4. **← 当前位置（NO-GO 后的新路线）**：
   ...
   n. **呈现有效性轴落地 + 任务5 权重首拟合 ✅（2026-06-12 下午）**：
      ...
      ②糊弄视频的晃动未被识别（youkoubo-c1 S1 给 2 分）——晃动是帧间差异，可由代码
      确定性计算（ffmpeg/帧差），比指望模型看出来更对路；③skincare S4 模型给 c2/b2 平局
      （事实层分歧）。下一步候选：a) 7 样本 repeat5 出稳定口径（35 调用）；
      b) 晃动改确定性信号进事实层（零 LLM）；c) 攒下批样本做拟合后的真泛化验证。
