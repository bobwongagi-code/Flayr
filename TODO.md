...
4. **← 当前位置（NO-GO 后的新路线）**：
   a. **修 task_completion 枚举（最优先，T5 重大发现）**：模型原始输出是自由文本
      （'both_complete'/'completed'/'full'…），normalize_choice fallback 把一切压成
      "partial"——此前"全是 partial"的观察是 fallback 伪影，#1/#3 的 gating 方案地基不存在。
      修法：prompt/schema 强制三值枚举 + 校验，'both_complete' 类语义错配修正；修完才谈门控。
   b. prompt 校准迭代：S5 判例接品类权重条件（低客单价无背书=正常）、两极逻辑/硬推判例
      缩窄适用条件；每次改动跑 gate 回归（装置现成：dev_run_gate.sh，一轮 15 调用）。
