...
4. **← 当前位置（NO-GO 后的新路线）**：
   ...
   d. **→ 架构提案（待用户裁决）**：severity 改为从稳定事实层确定性推导，不再由模型直出。
      依据：事实稳（S1/S3 全满票、tasha S5/S6 'creator_only'×5——模型对"谁做了什么"
      极其可靠）、判断飘且调不动；极性可机械消灭（gap_direction=creator_advantage→small）；
      品类权重做成数据表，校准循环变零 LLM 成本（对存量 facts 离线迭代）。
