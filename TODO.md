...
4. **← 当前位置（NO-GO 后的新路线）**：
   ...
   g. **下一迭代（待开工）**：① C 系数去词法化——新增逐阶段模型事实字段 painpoint_relevance
      （benchmark_only|creator_only|both|none），代码按枚举定 C，预期把 oracle 15/18 变成实跑成绩；
      ② 执行分锚点校准：0.5 档定义扩为"做了但对该阶段核心功能基本无效/平庸无感"（kakwan S1
      达人平铺直叙、are_xie S4 口头承诺均被宽给 1 分）；③ kakwan S3 镜头语言盲区三轮一致，
      属事实提取缺观察维度（取景完整性/歪斜），归 observation-guide。
