...
from . import frozen_inputs as frozen_inputs_module
from .api import (
...
from .frozen_inputs import (
    MAX_STAGE_FRAME_INPUTS_PER_STAGE,
...
def _read_json(path: Path, *, required: bool = True) -> dict[str, Any]:
    return frozen_inputs_module._read_json(path, required=required)


def _compact_fact_unit(unit: dict[str, Any]) -> dict[str, Any]:
    return frozen_inputs_module._compact_fact_unit(unit)


def _facts_for_role(run_dir: Path, role: str) -> dict[str, Any]:
    return frozen_inputs_module._facts_for_role(run_dir, role)


def _stage_code(stage: str) -> str:
    return frozen_inputs_module._stage_code(stage)


def _allowed_evidence_ids(facts: dict[str, Any]) -> dict[str, set[str]]:
    return frozen_inputs_module._allowed_evidence_ids(facts)


def _stage_frame_inputs(run_dir: Path, role: str) -> list[dict[str, str]]:
    return frozen_inputs_module._stage_frame_inputs(
        run_dir,
        role,
        image_to_data_url_fn=image_to_data_url,
    )


def _resolve_frozen_visual_path(raw_path: str, run_dir: Path) -> Path:
    return frozen_inputs_module._resolve_frozen_visual_path(raw_path, run_dir)


def _stable_digest(value: Any) -> str:
    return frozen_inputs_module._stable_digest(value)


def _file_digest(path: Path) -> str:
    return frozen_inputs_module._file_digest(path)


def _stage_time_ranges(run_dir: Path) -> dict[str, dict[str, str]]:
    return frozen_inputs_module._stage_time_ranges(run_dir)
