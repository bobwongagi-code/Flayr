...
def build_llm_comparison_payload(
    ...
    user_text = "\n\n".join(
        [
            ...
            "0.5 档同样适用于'内容存在但消费者无法有效接收'：看不清（虚焦/过曝/遮挡/一闪而过）、听不清（吞字/被 BGM 压制）、读不完（字幕停留过短）——物理存在不等于有效传递。S5 背书孤证规则：仅口播提及背书而画面无任何佐证、或背书标志一闪而过无法辨认，执行分最高 0.5（高决策门槛品类口头孤证视为无效背书）。",
