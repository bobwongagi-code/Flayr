...
def build_llm_comparison_payload(
    ...
    user_text = "\n\n".join(
        [
            ...
            "必须输出：one_line_verdict, one_line_summary, executive_summary, holistic_assessment（每维独立）, key_conclusions（1-5 条消费者视角）, product_visibility, category_profile, loop_closure, video_understanding, stage_analysis[6], improvements（1-5 条，按 GMV 杠杆排序）。",
            "stage_analysis 每项必须含：stage, time_range, benchmark_time_range, creator_time_range, core_question, creator_module_id, benchmark_module_id, module_fit, module_fit_reason, task_completion, gap_type, gap_summary, voice_performance, benchmark_summary, benchmark_key_message, benchmark_evidence_ids, benchmark_visual_evidence, benchmark_support_status, benchmark_quote, benchmark_quote_zh, creator_summary, creator_key_message, creator_evidence_ids, creator_visual_evidence, creator_support_status, creator_quote, creator_quote_zh, gap, evidence, severity, creator_execution, benchmark_execution, painpoint_relevance。S4 阶段额外含 creator_s4_effect_type 和 benchmark_s4_effect_type。",
            "category_profile 额外含 effect_visibility（该品效果可视化难度：high=用了立刻看得见如清洁去污/美妆变妆；medium=需时间或对比如散粉控油/护肤；low=感受型不可见如保健/口味）。",
            "S4 效果呈现【类型选择层】：两侧各输出 creator_s4_effect_type / benchmark_s4_effect_type，判该侧 S4 用了哪种**效果呈现**打法——before_after（使用前后同位置对比）、closeup_contrast（微距特写看细节变化）、person_contrast（用了 vs 没用对比）、prop_contrast（产品 vs 替代方案）、reference_object（参照物量化如吸油纸/水珠）、process_visualization（慢镜/微距展示肉眼难见的效果）、none。",
            "**S3/S4 类型铁律**：S4 效果呈现 = '用了**之后**的结果/变化'。使用动作本身（涂抹、按压、操作、喷洒）属于 S3 使用过程，**不算 S4 效果呈现**；口播说效果（'效果超好''控油一整天'）也不算。若某侧只有使用动作/口播说效果、没有展示'用了之后的真实结果变化'，creator/benchmark_s4_effect_type = none。这是判断的关键：先问'这个品 S4 该呈现什么效果'（参照 effect_visibility），再看该侧到底有没有做到。",
            ...
            "演示类阶段（S2-S4）的呈现有效性锚点：2 分要求关键环节可视化到位＋使用效果突出有对比＋场景丰富；过长的全程记录不加分（标尺是关键环节覆盖，如涂抹有、推开镜头有即够，不是完整性）。仅口头描述/文字叠参数式的卖点罗列、场景单一、缺效果对比——这类'做了但呈现单薄'最高 1 分，不得与多场景可视化演示打平。",
