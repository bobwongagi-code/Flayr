...
def build_llm_comparison_payload(
    ...
    user_text = "\n\n".join(
        [
            ...
            "stage_analysis 每项必须含：stage, time_range, benchmark_time_range, creator_time_range, core_question, creator_module_id, benchmark_module_id, module_fit, module_fit_reason, task_completion, gap_type, gap_summary, voice_performance, benchmark_summary, benchmark_key_message, benchmark_evidence_ids, benchmark_visual_evidence, benchmark_support_status, benchmark_quote, benchmark_quote_zh, creator_summary, creator_key_message, creator_evidence_ids, creator_visual_evidence, creator_support_status, creator_quote, creator_quote_zh, gap, evidence, severity, creator_execution, benchmark_execution, painpoint_relevance。S4 阶段额外必须含 creator_s4_evidence 和 benchmark_s4_evidence。",
            "S4（效果呈现）专项：两侧各输出 creator_s4_evidence / benchmark_s4_evidence，对该侧**视觉证据**做客观清点（不评价好坏）：has_side_by_side（前后同框/拼屏对比 true/false）、has_macro_detail（微距特写看到质感/纹理改变 true/false）、has_instrument_proof（仪器/工具/第三方介入如试纸紫外灯检测 true/false）、has_process_reveal（效果动态发生的瞬间、非静态结果图 true/false）、proof_strength（weak/moderate/strong，效果本身的视觉强度）。",
            "**S3/S4 隔离铁律**：清点 S4 证据和判 proof_strength 时**只看视觉画面**——口播里说的效果（如'99.9%杀菌''用了变白''效果超好'）一律不算 S4 证据，光说不算、要眼见为实。口播的效果声明归 S3 承诺，不进 S4 清点。",
