...
def write_analysis_input(run_dir: Path, analysis: dict[str, Any]) -> Path:
    ...
    lines = [
        ...
        "14b3. 演示类阶段（S2-S4）执行分以 product_profile.core_visual_proposition（本品核心视觉命题）为锚点，不套通用 before/after：先判该侧有没有拍出本品的决定性瞬间（定妆粉饼=油光→哑光对比、面膜=逐日变化+敷后效果），并满足 shooting_requirement（效果细微的品需正面强光+特写才算拍到）；拍出命题且拍摄到位才给 2，只完成动作（揭膜/擦粉/口头带过）未体现命题、或拍摄条件不支撑（暗光/无特写看不出效果）按敷衍计最高 0.5，做了但缺命题对比的'呈现单薄'最高 1。过长全程记录不加分（标尺是命题覆盖非完整性）。",
        "14b4. S2-S4 给执行分前必须做一次闭环核验：回到该侧关键帧，对照 core_visual_proposition 与 visual_diff_dimensions，在画面上实际确认那个视觉对比肉眼可见——'存在 before/after 结构'不等于'对比拍出来了'。若该侧前后帧在指定维度上看不出明显差异（油光帧与哑光帧看起来差不多、敷膜前后肤质无变化），即命题未被有效呈现，该侧执行分最高 1（只完成动作未呈现效果），几乎完全无差异则 0.5。把你自己定的命题当检查清单逐帧核对，不许凭结构臆断。两侧同此核验。",
        "14b5. S2-S4 执行分主轴只有一个：core_visual_proposition（核心命题）的有效呈现。trust_multipliers（防水/防汗测试、美容仪、周期记录、专业手法等）是加分项，只能在核心命题已有效呈现（该侧≥1）时把分抬向 2；不能替代、也不能补偿弱核心命题。若某侧核心命题没拍出来（对比弱/不可见），哪怕它有很强的次要演示，该侧执行分仍封顶 1——严禁用次要演示把分顶上去。先看核心命题达没达到，再决定加分项加不加。",
        ...
        "14d. 打分前先输出 product_profile 产品商业 DNA（这是 S2-S4 打分的尺子，先立尺再量）：visualizable（yes/no 核心价值能否视觉化）、physical_task（解决的最直观尴尬）、core_visual_proposition（决定性视觉瞬间=本品到位效果展示的标准，按本品现推，别套通用 before/after）、visual_diff_dimensions（本品 before/after 应在哪些视觉维度变化，从 亮度反光/纹理毛孔/色泽均匀度/水润干燥/肿胀轮廓 中选或按品自命名如去污/拉丝，1-3 个，S4 核验对比只看这些维度）、trust_multipliers（建立专业度的元素如美容仪/周期记录/第三方检测，3-6 个）、shooting_requirement（卖点显现所需拍摄条件）、confidence（high/low，小众或本地新奇特品标 low）。只报产品事实与品类世界知识。visualizable=no（香水/保健品/隐形矫正等效果拍不出）时 S2-S4 不强求视觉命题，把判断重心转到 S5 信任放大与达人可信度。",
