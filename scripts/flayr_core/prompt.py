...
def write_analysis_input(run_dir: Path, analysis: dict[str, Any]) -> Path:
    ...
    lines = [
        ...
        "14b3. 效果呈现阶段（S4）执行分以 product_profile.visual_proof_points 的 primary 证明点为主锚；旧结果无该字段时回退 core_visual_proposition。不套通用 before/after：先判该侧有没有拍出本品 primary 效果证明（清洁工具=清洁结果可见，美妆=指定视觉变化），并满足 shooting_requirement；secondary 证明点只能作为补充，不得替代 primary。若 primary 文本误把多个卖点写成复合条件，按最核心的消费者最终结果判断 primary，把机制/附加卖点缺失记为 secondary 不足。",
        ...
        "14d. 打分前先输出 product_profile 产品商业 DNA（这是 S1-S6 打分的尺子，先立尺再量）：visualizable、physical_task、hook_proposition、core_visual_proposition（旧兼容字段）、visual_proof_points（S4 多视觉证明点，数组 1-4 个；每项含 priority primary|secondary、proof_target、visual_standard、visual_diff_dimensions、related_selling_points；primary 是用户最核心效果证明且只能是一个消费者最终结果，禁止把多个卖点合并成 all-of；secondary 是附加卖点证明，不能压过 primary）、visual_diff_dimensions（旧兼容字段）、trust_multipliers、shooting_requirement、confidence。只报产品事实与品类世界知识。visualizable=no 时 S4 不强求视觉命题，把判断重心转到 S5 信任放大与达人可信度。",
