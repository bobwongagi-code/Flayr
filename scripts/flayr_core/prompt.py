...
def write_analysis_input(run_dir: Path, analysis: dict[str, Any]) -> Path:
    ...
    lines = [
        ...
        "14b4. S4 给执行分前必须做一次闭环核验：回到该侧关键帧，对照 core_visual_proposition 与 visual_diff_dimensions，在画面上实际确认那个视觉对比肉眼可见——'存在 before/after 结构'不等于'对比拍出来了'。若该侧前后帧在指定维度上看不出明显差异（油光帧与哑光帧看起来差不多、敷膜前后肤质无变化），即命题未被有效呈现，visual_difference_observed=false，该侧执行分最高 1（只完成动作未呈现效果），几乎完全无差异则 0.5。把你自己定的命题当检查清单逐帧核对，不许凭结构臆断。两侧同此核验。还必须按 structure_library_full.md 的 S4-A~F 模块硬约束判断 module_constraints_met；模块硬约束不成立，不能给满执行。",
