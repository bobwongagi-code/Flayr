...
def write_analysis_input(run_dir: Path, analysis: dict[str, Any]) -> Path:
    ...
    lines = [
        ...
        "14. severity 是模型对本阶段差距的初步判断，不要求人为凑出差异化分布。后处理只会在显式、可追溯事实满足已验证规则时，用 floor/ceiling 收窄模型结果；缺失、unknown 或 uncertain 不触发规则。gap_summary 写‘无明显差距’时仍应如实保留模型判断，不要为了迎合文字强行改 severity。",
        ...
        "14b. 每阶段必须输出 creator_execution 和 benchmark_execution 两个独立执行分，取值只能是 0、0.5、1、2 四个数字：0=未执行该阶段功能；0.5=做了但对核心功能基本无效——敷衍、平庸无感（如一句轻带的 CTA、平铺直叙的开场、仅口头承诺无验证）；1=执行合格（功能完成且有效）；2=执行出色。两侧各自按该阶段功能定义独立打分，先打分再对比，禁止因对比结果回调分数；这些分数只作为诊断和审计观察，不直接决定最终 severity。",
        ...
        "14b3. 效果呈现阶段（S4）只锚 short_video_proof_plan 选中的 S4 candidate 所生成的 visual_proof_points.primary；它是单一可测信号，不代表产品只有一个卖点。其它卖点按计划留在 S2/S3/S5，不能替代 S4 anchor；旧结果无该计划时才回退 core_visual_proposition。",
        "14b4. S4 给执行分前必须做一次闭环核验：回到该侧关键帧，对照 visual_proof_points.primary.visual_standard/visual_diff_dimensions（无该字段则用 core_visual_proposition 与 visual_diff_dimensions），在画面上实际确认那个视觉对比肉眼可见——'存在 before/after 结构'不等于'对比拍出来了'。若 primary 指定维度看不出明显差异，visual_difference_observed=false，该侧执行分最高 1；几乎完全无差异则 0.5。secondary 不得补偿 primary 缺失。",
        "14b5. S4 执行分主轴是 primary visual proof 的有效呈现。trust_multipliers 与 visual_proof_points.secondary 是加分项或补充说明，只能在 primary 已有效呈现（该侧≥1）时把分抬向 2；不能替代、也不能补偿弱 primary。",
        ...
        "14b2. 每阶段必须输出 painpoint_relevance（benchmark_only/creator_only/both/none 四选一）：该阶段双方内容是否命中 category_profile.painpoints 的核心决策因素，按内容功能判断（讲没讲到、演没演到核心痛点），不要求字面用词一致。它只供 commercial_priorities 在同一 severity 层级内排序；缺失、unknown 或无法判断时留空，不等同于 none，也不参与 severity。",
    ...
    rendered = apply_certification_ownership_policy("\n".join(lines)).rstrip() + "\n"
