...
量三件（见讨论）：
  ...
  3. landing_window_leak 频率：landing=true 但 landing_reason 引用了超过钩子窗口（默认 6s）
     的时间戳 → 疑似"用后段痛点/产品补足三件套"（carslan 失败模式探针）
...
def audit_sample(run_dir: Path, pattern: str, cutoff: float) -> dict[str, Any] | None:
    ...
    for side in ("creator", "benchmark"):
        ...
        # 窗口泄漏：landing=true 且 reason 引用时间戳 > cutoff
        leaks = [
            h.get("landing_met") is True and _max_ts(h.get("landing_reason") or "") > cutoff
            for h in hooks
        ]
        rows[side] = {
            "n": len(hooks),
...
def main() -> None:
    ...
    print(f"窗口泄漏判据：landing=true 且 landing_reason 引用时间戳 > {args.cutoff}s\n")
    for run_dir in dirs:
        rows = audit_sample(run_dir, args.glob, args.cutoff)
        if not rows:
        ...
        for side, r in rows.items():
            print(f"  {side:9} n={r['n']} | type={r['type'][0]}({r['type'][1]}) "
                  f"landing={r['landing'][0]}({r['landing'][1]}) anchors={r['anchors'][0]}({r['anchors'][1]})")
